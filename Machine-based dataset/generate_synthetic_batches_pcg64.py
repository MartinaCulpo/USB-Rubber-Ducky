...
# === PRNG: PCG64
def prng_pcg64(seed, size=1):
    return np.random.Generator(np.random.PCG64(seed)).random(size)
    ...
    for batch_num in range(num_batches):
        ...
        for sid, sentence in enumerate(batch_sentences):
            ...
            for i, c in enumerate(sentence):
                ...
                hold = int(hold_range[0] + prng_pcg64(sentence_id + i, 1)[0] * (hold_range[1] - hold_range[0]))
                flight = int(flight_range[0] + prng_pcg64(sentence_id + i + 1000, 1)[0] * (flight_range[1] - flight_range[0]))
